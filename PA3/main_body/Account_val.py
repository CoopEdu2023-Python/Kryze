...
# 评估视频评论区和视频数据
def video_state_score(data_video: list, data_stat: list, progress_callback=None):
    sum = 1
    res = []
    val_list = []
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        future_to_segment = {executor.submit(predict_process, segment): segment for segment in segments}

        for future in tqdm(concurrent.futures.as_completed(future_to_segment), total=len(segments), mininterval=0.1):
            segment = future_to_segment[future]
            try:
                segment_result = future.result()
                val_list.append(segment_result)
                if progress_callback:
                    progress_callback(len(val_list), len(segments))
...
def generate_weight(name: str):
    ...
    res = video_state_score(reply_list, stat_list, progress_callback=update_progress)
